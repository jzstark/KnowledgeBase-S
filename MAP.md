...
| 路径 | 说明 |
|---|---|
| `docker-compose.yml` | 生产部署：api（`RUN_MIGRATIONS=1`，唯一 migrator）/ web / kb-mcp / 可选 kb-mcp-oauth / workers / postgres / nginx / watchtower。workers 用 `workers` profile，OAuth MCP 用 `oauth` profile；两个 MCP 默认使用 `stable` 镜像 tag、排除 Watchtower，并保留 SHA 回滚能力 |
