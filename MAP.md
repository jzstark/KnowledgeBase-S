...
| `services/api/routers/folders.py` | **Phase B 新增**：文件夹 / 文档实例 / Connector API（三个 sub-router）；含文档级硬删除 `_hard_delete_document_instance`，以及单篇/批量持久化重新生成入口（批量按真实 source 合并触发） |
| `services/web/` | Next.js 前端：知识图谱可视化、资料夹文件管理器 |
| `services/web/app/sources/page.tsx` | **Phase B 新建**：三栏文件管理器 UI（资料夹树 + 内容列表 + 详情抽屉）；支持搜索、多选、批量归档/改类型/重新生成及有限状态轮询 |
...
| `deploy.sh` | VPS 部署脚本：默认维护 core + workers + OAuth MCP；兼容原有 `./deploy.sh --oauth` 写法；不自动删除可选 profile 或回滚镜像 |
