...
| Source 详情页编辑 `default_doc_kind` | `sources/[id]/page.tsx` 新增默认类型下拉，调用 `PUT /api/sources/{id}` |
| Source items 手动覆盖 `doc_kind` | `PATCH /api/sources/source-items/{item_id}` + Source 详情页 item 下拉；已入库 article node 同步更新 |
...
| 资源管理器按 source 分组 | `knowledge/page.tsx` ListPanel `groupBySrc` toggle，后端 `/api/kb/nodes` 返回 `source_name` |
| 已停用 source 分组标注 | `/api/kb/nodes` 返回 `source_deleted_at`，分组标题显示 `[已停用]` |
| 资源列表按知识时间排序 | `/api/kb/nodes` 改为按 `COALESCE(published_at, ingested_at, created_at)` 排序 |
...
- `routers/sources.py`：`SourceCreate / SourceUpdate / SourceItemCreate` 加 `default_doc_kind / doc_kind`；`/upload` 接 Form `doc_kind`；`/add-url` 接 body `doc_kind`；`update_source` 支持 PUT `default_doc_kind`；新增 `PATCH /source-items/{item_id}` 覆盖 item `doc_kind`
...
| TypeScript 编译（审计修复后） | `npx tsc --noEmit` 干净 |

**当前剩余项**

- **Source 删除 cascade 选项**：当前 `DELETE /api/sources/{id}` 仅软删除 source，保留 articles；尚未提供“同时删除 N 篇文章”的 cascade 选项。
- **Source 管理页显示已停用 source**：后端 `GET /api/sources?include_deleted=true` 已支持，但 `/sources` 页面默认仍只展示 active sources；如需恢复/管理停用 source，需要增加前端切换。
